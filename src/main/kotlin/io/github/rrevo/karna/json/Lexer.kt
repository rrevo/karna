...
//        log("Next char: '$c' index:$index")
    ...
    private fun actualNextToken() : Token {
        ...
        if ('"' == c || (lenient && expectName)) {
            ...
            do {
                if (isDone) {
                    throw KarnaException("Unterminated string")
                }
                        ...
                        when (c) {
                            ...
                            'u' -> {
                                ...
                                try {
                                    ...
                                    throw KarnaException("EOF reached in unicode char after: u$unicodeChar")
                                }

                                val intValue = try {
                                    java.lang.Integer.parseInt(unicodeChar.toString(), 16)
                                } catch(e: NumberFormatException) {
                                    throw KarnaException("Failed to parse unicode char: u$unicodeChar")
                                }
            ...
            if (NUMERIC.matcher(v).matches()) {
                ...
                throw KarnaException("Unexpected character at position ${index-1}"
                        + ": '$c' (ASCII: ${c.toInt()})'")
            }
