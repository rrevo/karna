...
import io.github.rrevo.karna.json.internal.firstNotNullResult
import java.util.*
...
//                    parameterMap[parameter] = adjustType(convertedValue, parameter)
        ...
        if (errorMessage.any() && ! success) {
            throw KarnaException(errorMessage.joinToString("\n"))
        }
        ...
        return result ?: throw KarnaException(
...
//    }
        ...
        allProperties.forEach { thisProp ->
            ...
            if (path == null) {
                ...
                result[prop.name] = allPaths[path]
                        ?: throw KarnaException("Couldn't find path \"$path\" specified on field \"${prop.name}\"")
    ...
    private fun illegalPropField(prop: String, field: String): Nothing {
        throw KarnaException("The @TypeFor annotation on field \"$prop\"" +
                " refers to nonexistent field \"$field\"")
    }

    private fun illegalPropClass(field: String, className: String): Nothing {
        throw KarnaException("The @TypeFor annotation on class \"$className\"" +
                " refers to nonexistent field \"$field\"")
    }
}
